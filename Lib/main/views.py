    ...
    if request.user.is_authenticated:
        ...
        if currentUser.is_staff:
            ...
            for typeItem in BookType.objects.all():
                typeInfo.append({"typeNum": typeItem.TypeNum,
                                 "typeInfo": typeItem.TypeName})
            currentUserInfo = {"userId": currentUser.username,
                               "userName": staffInfo.staffName}
            # To staffMain
            return render(request, 'indexStaff.html', {"currentUser": currentUserInfo, "is_staff": True, "typeList": typeInfo})
        else:
    ...
    if request.user.is_authenticated:
        ...
        if currentUser.is_staff:
            try:
                ...
                userInfoFK = UserInfo.objects.get(
                    userId=authUser.objects.get(username=userid))
    ...
    if request.user.is_authenticated:
        ...
        if mode == 'Detail':
            ...
            if bookLikeName == '':  # 一连串书的信息
                ...
                BookAll = Book.objects.filter(
                    bookName__contains=bookLikeName).order_by("bookName")
            ...
            if currentPage-1 > after_range_num:
                pageList = bookPagi.page_range[currentPage-1 -
                                               after_range_num:currentPage-1+befor_range_num]
            else:
            ...
            if currentUser.has_perms(["main.add_additem", "main.change_additem", "main.add_booknum", "main.change_booknum"]):
                try:
                    ...
                    AddItem.objects.create(
                        bookId=BookFK, staffId=staffInfoFK, addNum=addBookNum)
                    try:
                        changeBookNum = BookNum.objects.get(
                            bookId__bookId=bookid)
                        changeBookNum.bookNum += int(addBookNum)
                        changeBookNum.save()
                    except:
                        BookNum.objects.create(
                            bookId=BookFK, bookNum=addBookNum)
        ...
        if currentUser.is_staff:
            if currentUser.has_perms(["main.add_borrowitem", 'main.change_booknum']):
                ...
                borrowitems = BorrowItem.objects.create(borrowItemId=borrowitemsNum+1,
                                                        ...
                                                        staffId=StaffInfo.objects.get(
                                                            staffId=currentUser),
                                                        userId=UserInfo.objects.get(
                                                            userId__username=userid),
                                                        hasReturned=False)
        ...
        if currentUser.is_staff:
            if currentUser.has_perms(["main.change_borrowitem"]):
                try:
                    ...
                    return HttpResponse('get error')
                bookNumItem = BookNum.objects.get(bookId__bookId=bookid)
                borrowitems = BorrowItem.objects.filter(
                    bookId__bookId=bookid, userId__userId__username=userid).exclude(hasReturned=True)
...
def staffViewUserDetail(request):
    if request.user.is_authenticated:
        ...
        if currentUser.is_staff:
            ...
            borrowList = BorrowItem.objects.filter(
                userId=userInfoFK).order_by('borrowItemId')
...
def staffViewUser(request):
    if request.user.is_authenticated:
        ...
        if currentUser.is_staff:
            ...
            if currentPage-1 > after_range_num:
                pageList = userPagi.page_range[currentPage-1 -
                                               after_range_num:currentPage-1+befor_range_num]
            else:
                pageList = userPagi.page_range[0:currentPage-1+befor_range_num]
            ans = {'userList': ansUserList,
...
# http://127.0.0.1:8000/getIdd_CountryInfo?offset=100&startPage=1


def getIdd_CountryInfo(request):
    offset = request.GET['offset']
    startPage = request.GET['startPage']
    currentPage = int(startPage)
    ...
    ans = {'countryList': ansCountryList}
    return HttpResponse(json.dumps(ansCountryList,  ensure_ascii=False), content_type="application/json")
